...
const formReducer = (data = initial, action) => {
    switch (action.type) {
        ...
        case 'INITIAL': return { name: '', weight: '', img: ''}
        case 'CHANGE':
            console.log(action)
            return {
                ...data,
...
const bearReducer = (bears = [], action) => {
    ...
    switch (action.type) {
        ...
        case 'ADD_BEARS': return [...bears, { id: bears.length, ...action.bear }]
        case 'DELETE_BEARS': return bears.filter(bear => +bear.id !== +action.id)
        case 'UPDATE_BEARS': return bears.map(bear => {
